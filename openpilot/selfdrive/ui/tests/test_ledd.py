...
def test_all_camera_led_buses_are_configured():
  leds = CameraLeds()
...
def test_status_colors_scale_to_requested_brightness():
...
def test_userspace_controls_every_kernel_led_channel(tmp_path, monkeypatch):
...
def test_kernel_boot_blink_runs_until_first_openpilot_state(tmp_path, monkeypatch):
  monkeypatch.setattr(ledd, "CAM_LED_SYSFS_ROOT", tmp_path)
  channel_paths = []
  for camera_num in (1, 2, 3):
...
def test_status_policy_keeps_cam1_off_but_explicit_channels_can_enable_it(monkeypatch):
  leds = CameraLeds()
  states = {}
...
def test_missing_camera_boards_do_not_stop_ledd(missing_count, monkeypatch):
  leds = CameraLeds()
  updated = []
  ...
  for board in leds.boards[missing_count:]:
    monkeypatch.setattr(board, "set", lambda _state, name=board.name: updated.append(name))

  leds.set(LedState("test", 0, 0, 180), force=True)
  assert updated == [board.name for board in leds.boards[missing_count:]]


def test_pairing_feedback_exclusively_controls_all_camera_leds(monkeypatch):
...
def test_pairing_color_code_uses_six_distinct_leds_on_cam2_and_cam3(monkeypatch):
...
def test_camera_led_brightness_matches_openpilot_screen_mapping(exposure_percent, expected):
  class SubMaster:
    ...
    def __getitem__(self, name):
      ...
      return SimpleNamespace(exposureValPercent=exposure_percent)

  assert ledd.camera_led_brightness(SubMaster()) == expected


def test_camera_led_brightness_falls_back_to_road_then_startup():
  class SubMaster:
...
def test_update_led_blinks_violet_for_active_update_phases():
...
def test_disabled_pandad_is_not_disconnected():
...
def test_enabled_pandad_without_state_is_disconnected():
