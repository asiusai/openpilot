...
def test_advertisement_uses_product_name():
  assert bled.Advertisement().LocalName == bled.ASIUS_DEVICE_NAME


@pytest.mark.asyncio
async def test_advertisement_refresh_reregisters_after_missing_registration(monkeypatch):
  calls = []
...
@pytest.mark.asyncio
async def test_pairing_approval_waits_for_active_peer(monkeypatch):
  approved = {
    "status": "approved",
    "publicKey": APP_KEY,
    "requestId": "request-1",
...
@pytest.mark.asyncio
async def test_pairing_approval_is_retained_until_delivered(monkeypatch):
  approved = {
    "status": "approved",
    "publicKey": APP_KEY,
    "requestId": "request-1",
...
@pytest.mark.asyncio
async def test_authorized_pairing_retry_delivers_and_clears_approval(monkeypatch):
  approved = {
    "status": "approved",
    "publicKey": APP_KEY,
    "requestId": "request-1",
...
@pytest.mark.asyncio
async def test_authenticated_pairing_error_preserves_request_id(monkeypatch):
  engine = make_engine()
  errors = []
  monkeypatch.setattr(bled, "unpack_peer_message", lambda _payload, _recipient, validate_timestamp=True: (
    APP_KEY,
...
@pytest.mark.asyncio
async def test_unauthenticated_payload_gets_no_error(monkeypatch):
  engine = make_engine()
  errors = []
...
@pytest.mark.asyncio
async def test_ble_message_ignores_wall_clock(monkeypatch):
  engine = make_engine()
  handled = []
...
@pytest.mark.asyncio
async def test_ble_response_uses_peer_timestamp(monkeypatch):
  engine = make_engine()
...
@pytest.mark.asyncio
async def test_ble_pairing_expiry_uses_peer_time(monkeypatch):
  engine = make_engine()
  engine.peer_clocks[APP_KEY] = (1_700_000_000, 100.0)
  sent = []
  monkeypatch.setattr(bled.time, "monotonic", lambda: 105.0)
